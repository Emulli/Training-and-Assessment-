...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($result->num_rows === 1) {
        ...
        // Verify Password (Plain text for now, match your register logic)
        if ($password === $user['password']) {
            
            // Check if account is rejected
            if ($user['status'] == 'rejected') {
                $error = "Your account has been deactivated or rejected.";
            } else {
                // === SUCCESS ===
                ...
                // Redirect to the Dashboard (Where they see status/courses)
                header("Location: training/training.php");
                exit();
            }
        } else {
...
<body class="bg-gradient-to-br from-blue-500 via-cyan-800 to-indigo-900 min-h-screen flex items-center justify-center p-4 relative overflow-hidden">
    ...
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-4xl overflow-hidden flex flex-col md:flex-row relative z-10 min-h-[500px]">
        ...
        <div class="hidden md:flex w-1/2 bg-gradient-to-br from-violet-700 to-cyan-800 text-white p-8 flex-col justify-center relative overflow-hidden">
            ...
            <div class="relative z-10 text-center">
                <div class="mx-auto bg-white p-2 w-28 h-28 rounded-full flex items-center justify-center mb-6 shadow-xl ring-4 ring-white/20">
                    <img src="/OJT/assets/images/TVLC_log.jpg" alt="TVLC Logo" class="w-full h-full object-contain rounded-full">
                </div>
        ...
        <div class="w-full md:w-1/2 p-6 md:p-6 bg-white flex flex-col justify-center relative">
            ...
            <div class="md:hidden text-center mb-6">
                <div class="w-16 h-16 mx-auto mb-2 bg-primary rounded-full flex items-center justify-center text-white text-2xl">🎓</div>
            ...
            <?php if (!empty($error)): ?>
                <div class="mb-4 p-3 bg-red-50 text-red-500 text-sm rounded-lg border border-red-100 text-center font-bold">
                    <?php echo $error; ?>
                </div>
            <?php endif; ?>
            ...
            <form method="POST" class="space-y-5">
                ...
                <div>
                    ...
                    <div class="relative group">
                        ...
                        <input type="email" name="email" 
                               ...
                               class="w-full pl-10 pr-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:bg-white focus:ring-2 focus:ring-primary focus:border-transparent outline-none transition-all" 
                ...
                <div class="flex items-center justify-between text-sm">
                    <label class="flex items-center text-gray-500 hover:text-gray-700 cursor-pointer">
                        <input type="checkbox" name="remember" class="w-4 h-4 text-primary border-gray-300 rounded focus:ring-primary" <?php echo isset($_COOKIE['user_email']) ? 'checked' : ''; ?>>
                        <span class="ml-2 font-medium">Remember me</span>
                    </label>
                    <a href="#" class="text-gray-400 hover:text-primary font-medium transition-colors">Recovery Password</a>
                </div>
