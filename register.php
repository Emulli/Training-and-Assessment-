...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Combine names to match database schema
    ...
    if ($password !== $confirm_password) {
        ...
        if ($stmt->num_rows > 0) {
            ...
            // 3. Insert User (Status is 'active' so they can login immediately)
            $stmt = $conn->prepare("INSERT INTO users (name, email, password, status) VALUES (?, ?, ?, 'active')");
            $stmt->bind_param("sss", $fullname, $email, $password);

            if ($stmt->execute()) {
                $message = "Registration successful! Please log in to upload your requirements.";
                $msg_type = "success";
            } else {
    ...
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-5xl overflow-hidden flex flex-col md:flex-row relative z-10 min-h-[600px]">
        ...
        <div class="w-full md:w-7/12 p-8 md:p-12 bg-white flex flex-col justify-center">
            ...
            <?php if (!empty($message)): ?>
                <div class="mb-6 p-4 rounded-lg text-sm font-bold text-center <?php echo $msg_type == 'success' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'; ?>">
                    <?php echo $message; ?>
                </div>
            <?php endif; ?>
                ...
                <div class="flex items-start mt-2">
                    ...
                    <div class="ml-2 text-xs">
                        <label for="terms" class="text-gray-500">
                            By creating an account, I certify that I have read and agree to the 
                            <a href="#" class="text-primary font-bold hover:underline">Terms of Service</a> and 
